...
本项目基于 [FastAPI](https://fastapi.tiangolo.com/) 与 [HTTPX](https://www.python-httpx.org/) 的快速异步框架构建，提供抖音/TikTok 数据爬取的核心 API 服务，可批量解析并下载无水印视频或图集，你也可以在自己的项目中直接调用 [scraper.py](https://github.com/Evil0ctal/Douyin_TikTok_Download_API/blob/Stable/scraper.py) 或安装现有的 [pip 包](https://pypi.org/project/douyin-tiktok-scraper/) 来爬取数据。
...
启动后访问 `http://localhost:80/docs` 查看接口文档，或使用上文的 MCP 接口获取 OpenAPI 描述。
...
本项目支持 **Model Control Protocol (MCP)**，启动后可通过以下命令获取完整的
OpenAPI 描述，方便大模型等客户端发现接口：
